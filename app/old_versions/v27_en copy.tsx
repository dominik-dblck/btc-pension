...
const BTC_PensionCalculator: React.FC = () => {
  ...
  const platformAnnualSeries = useMemo(() => {
    ...
    // 1c) deployed capital snapshot per user (AUM)
          ...
          <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
            ...
            <CardContent className="px-6 pb-8 space-y-3">
              ...
              <p className="text-xs text-gray-400 pt-1">
                ...
                <em>Exchange Fee (€/year)</em>, <em>AUM (€/year)</em>,{' '}
                <em>Avg per user (€/year)</em> and <em>Users</em> (right axis).
                Fees calculated from yield (earn) + one‑off exchange fees
                (EUR→BTC purchases) — cohort convolution (users join over time).
              </p>
...
try {
  if (typeof window !== 'undefined') {
    ...
    console.assert(
      diff / Math.max(1, expectedReal) < 1e-9,
      'totalContribReal should include gross amounts when exchange fee > 0'
    );
  }
} catch {}
