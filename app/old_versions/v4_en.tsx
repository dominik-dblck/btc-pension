      ...
      >
        ...
        <header className="text-center space-y-4">
          ...
          <p className="text-sm md:text-base text-gray-300 max-w-3xl mx-auto">
            ...
            Fees: 1% platform & 1% referrer on <em>positive</em> user net
            P&amp;L. Values are pre-tax.
          </p>
        </header>
          ...
          <CardContent className="px-6 pb-8 space-y-6 text-sm">
            <div>
              ...
              <ul className="list-disc pl-6 space-y-1 text-gray-300">
                <li>
                  Monthly DCA in EUR; deterministic BTC CAGR{' '}
                  {(inp.cagr * 100).toFixed(1)}%.
                </li>
                <li>
                  Target LTV {(inp.ltv * 100).toFixed(0)}%, loan APR{' '}
                  {(inp.loanRate * 100).toFixed(1)}%, yield APY{' '}
                  {(inp.yieldRate * 100).toFixed(1)}%.
                </li>
                <li>
                  Rebalance to target LTV at the end of each quarter; interest
                  and yield accrue monthly.
                </li>
                <li>
                  Fee: 1% platform + 1% referrer on positive user net P&amp;L.
                </li>
              </ul>
            </div>
            {(spreadWarn || ltvWarn) && (
              <div className="text-xs text-amber-300">
                {spreadWarn && (
                  <p>
                    ...
                    conditions, spread income will not cover debt cost.
                  </p>
                )}
                {ltvWarn && (
                  <p>
                    Warning: LTV &gt; 80% — high operational risk (ignored here,
                    but flagged).
                  </p>
                )}
              </div>
        ...
        <section className="grid xl:grid-cols-3 lg:grid-cols-2 gap-10">
          ...
          <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
            <CardHeader className="px-6 pt-6 pb-3">
              ...
              <CardDescription className="text-xs text-gray-400">
                % fields are entered as <em>percent values</em>; model uses
          ...
          <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
            <CardHeader className="px-6 pt-6 pb-3">
              ...
              <CardDescription className="text-xs text-gray-400">
                1% fee after {inp.years} years vs number of users (on positive
                P&L)
              </CardDescription>
            </CardHeader>
            <CardContent className="px-6 pb-8 space-y-4 h-80">
              <div>
                <label
                  className="text-xs font-semibold text-gray-300"
                  title="Total number of active users at horizon"
                >
                  Max Users
                </label>
                <Input
          ...
          <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
            <CardHeader className="px-6 pt-6 pb-3">
              ...
              <CardDescription className="text-xs text-gray-400">
                1% share after {inp.years} years vs number of referrals (on
                positive P&L)
              </CardDescription>
            </CardHeader>
            <CardContent className="px-6 pb-8 space-y-4 h-80">
              <div>
                <label
                  className="text-xs font-semibold text-gray-300"
                  title="Number of direct referrals"
                >
                  Max Referrals
                </label>
                <Input
