...
const BTC_PensionCalculator: React.FC = () => {
  ...
  const platformFeePU =
    Math.max(0, platformSim[platformSim.length - 1].pnlNet) *
    (platformFeePct / 100);
  ...
  const platformAnnualSeries = useMemo(() => {
    // per-user przychód w danym roku
    let prevPeak = 0;
    const perYear = new Map<number, number>(); // year -> fee per user w tym roku
    for (const pt of platformSim) {
      ...
      if (inc > 0) {
        ...
        perYear.set(year, (perYear.get(year) ?? 0) + feeStep);
        prevPeak = pnlPlus;
      }
    }
    return Array.from({ length: inp.years }, (_, i) => {
      const y = i + 1;
      const perUser = perYear.get(y) ?? 0;
      return {
        year: y,
        perUser,
        total: perUser * maxUsers, // łączny przychód przy zadanej liczbie użytkowników
      };
    });
  }, [platformSim, platformFeePct, maxUsers, inp.years]);
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-black text-gray-100">
        ...
        >
            ...
            <CardContent className="px-6 pb-8 space-y-6 text-sm">
              <div>
                ...
                <ul className="list-disc pl-6 space-y-1 text-gray-300">
                  ...
                  <li>
                    <strong>Referrals</strong>: 1% fee od <em>pozytywnych</em>{' '}
                    inkrementów P&amp;L użytkowników poleconych zwiększa Twoje
                    saldo (parametry: liczba i średnia wpłata).
                  </li>
                  <li>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <LabelWithInfo
                        ...
                        tip="Kwota kredytu zabezpieczonego na BTC, pełne pokrycie według wybranego LTV."
                      />
                      <Input
                        type="number"
                        min={0}
                    ...
                    <div className="space-y-1">
                      <LabelWithInfo
                        text="Loan LTV (%)"
                        tip="LTV użyte do oceny zdolności: capacity = LTV × BTC value − istniejący dług."
                      />
                      <Input
                        type="number"
                        min={0}
          ...
          <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
            ...
            <CardContent className="px-6 pb-8 space-y-3">
              <p className="text-xs text-gray-400">
                Fee od P&amp;L po całym cyklu ({inp.years} lat) — ustaw{' '}
                <em>Users — Count</em>, <em>Users — Avg Monthly</em> i{' '}
                <em>Fee (%)</em>.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                ...
                <div className="space-y-1">
                  <LabelWithInfo
                    text="Fee (%)"
                    tip="Procent od dodatniego P&amp;L uczestnika pobierany przez platformę po zakończeniu cyklu."
                  />
                  <Input
                    type="number"
                    min={0}
              ...
              <div className="text-xs text-gray-300 pt-1">
                <span className="mr-4">
                  Per user (per cykl): <strong>€{fmt(platformFeePU)}</strong>
                </span>
                <span>
                  Łącznie przy <strong>{fmt(maxUsers)}</strong> users:{' '}
                  <strong>€{fmt(platformFeePU * maxUsers)}</strong>
                </span>
              </div>
          ...
          <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
            <CardHeader className="px-6 pt-6 pb-3">
              <CardTitle className="text-lg text-white">
                Platform Earnings — Over Time (Annual)
              </CardTitle>
            </CardHeader>
            <CardContent className="px-6 pb-8 space-y-3">
              <p className="text-xs text-gray-400">
                Roczne przychody z fee od dodatnich przyrostów P&amp;L
                (high‑water mark). Linie: per user oraz łącznie dla{' '}
                <strong>{fmt(maxUsers)}</strong> users. Fee:{' '}
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={platformAnnualSeries}>
                    ...
                    <Line
                      ...
                      strokeDasharray="4 2"
                      stroke="#06b6d4"
                      name={`Total (€/year) @ ${fmt(maxUsers)} users`}
                    />
                  </LineChart>
                </ResponsiveContainer>
              </div>
