...
const BTC_PensionCalculator: React.FC = () => {
  ...
  const simWithRef = useMemo(() => {
    ...
    return sim.map(pt => ({
      ...pt,
      netWorthWithRef: pt.netWorth + (feeByMonth.get(pt.month) ?? 0),
    }));
  }, [sim, refFeeSeries]);
  ...
  const totalRefIncome = refFeeSeries.length
    ? refFeeSeries[refFeeSeries.length - 1].feeCum
    : 0;
  ...
  const carM = useMemo(
    () => firstAffordableMonth(sim, carCost, inp.ltv),
    [sim, carCost, inp.ltv]
  );
  const houseM = useMemo(
    () => firstAffordableMonth(sim, houseCost, inp.ltv),
    [sim, houseCost, inp.ltv]
  );
  const tuitionM = useMemo(
    () => firstAffordableMonth(sim, tuitionCost, inp.ltv),
    [sim, tuitionCost, inp.ltv]
  );
              ...
              <div>
                ...
                <ul className="list-disc pl-6 space-y-1 text-gray-300">
                  ...
                  <li>
                    ...
                    ich wpłat: liczba i średnia miesięczna kwota).
                  </li>
                  <li>
            ...
            <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
              <CardHeader className="px-6 pt-6 pb-3">
                ...
                <CardDescription className="text-xs text-gray-400">
                  Pola % wpisuj jako wartości <em>w procentach</em>; model
                  przelicza je na ułamki.
                </CardDescription>
              </CardHeader>
              <CardContent className="px-6 pb-8 space-y-6">
                  ...
                  {(
                    [
                      [
                        ...
                        1,
                        1,
                        'Miesięczna wpłata w EUR, kupujemy BTC po cenie z danego miesiąca',
                      ],
                      [
                        'initialPrice',
                      ...
                      [
                        ...
                        0.1,
                        100,
                        'Roczna stopa wzrostu BTC w idealnym scenariuszu (bez zmienności)',
                      ],
                      [
                        'years',
                        'Horizon (yrs)',
                        1,
                        1,
                        'Horyzont oszczędzania w latach',
                      ],
                      [
                        'ltv',
                        'Max/Target LTV (%)',
                        0.1,
                        100,
                        'Docelowy/maksymalny LTV używany do liczenia pojemności długu',
                      ],
                      [
                        'loanRate',
                        'Loan Interest (APR, %)',
                        0.1,
                        100,
                        'Roczna stopa procentowa pożyczki',
                      ],
                      [
                        'yieldRate',
                        'Yield Rate (APY, %)',
                        0.1,
                        100,
                        'Roczna stopa zwrotu z ulokowanego kapitału (na długu)',
                      ],
                    ] as [
                ...
                <div className="grid grid-cols-2 gap-4 pt-4">
                  <div className="space-y-1">
                    <LabelWithInfo
                      ...
                      tip="Średnia miesięczna wpłata jednego poleconego. Służy do modelowania ich P&L i wynikającej z tego 1% prowizji."
                    />
                    <Input
                      type="number"
                      min={0}
                ...
                <div className="grid grid-cols-2 gap-2 pt-4 text-xs text-gray-300">
                  ...
                  <span>
                    ...
                    <strong>€{fmt(totalRefIncome)}</strong>
                  </span>
                  <span>
                    Net Worth + Referrals:{' '}
                    <strong>
            ...
            <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
              ...
              <CardContent className="px-6 pb-8 space-y-4 h-80">
                <div>
                  <LabelWithInfo
                    text="Max Users"
                    tip="Łączna liczba aktywnych użytkowników po horyzoncie — wykres przychodów platformy (1% od P&L)."
                  />
                  <Input
                    type="number"
                    min={1}
            ...
            <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
              ...
              <CardContent className="px-6 pb-8 space-y-4 h-80">
                <div>
                  <LabelWithInfo
                    text="Max Referrals"
                    tip="Liczba bezpośrednich poleconych — wykres przychodów referenta (1% od P&L użytkowników)."
                  />
                  <Input
                    type="number"
                    min={1}
...
function runSelfCheck() {
  ...
  console.assert(
    firstAffordableMonth(s, 1e15, base.ltv) === null,
    'huge cost should be unaffordable'
  );
  // referral series monotonic cumulative
  const fees = computeReferralFeeSeries(s, 3);
  for (let i = 1; i < fees.length; i++) {
    console.assert(
      fees[i].feeCum >= fees[i - 1].feeCum,
      'ref feeCum should be non-decreasing'
    );
  }
}
try {
