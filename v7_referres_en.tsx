        ...
        >
          ...
          <header className="text-center space-y-4">
            ...
            <p className="text-sm md:text-base text-gray-300 max-w-3xl mx-auto">
              Deterministic DCA → LTV-loan → yield model under ideal conditions. Thresholds for large purchases (car / house / tuition) are calculated from available loan capacity.
            </p>
          </header>
          ...
          <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-xl">
            ...
            <CardContent className="px-6 pb-8 space-y-6 text-sm">
              <div>
                <h3 className="font-semibold mb-1">Key assumptions (ideal conditions)</h3>
                <ul className="list-disc pl-6 space-y-1 text-gray-300">
                  ...
                  <li><strong>Rebalancing</strong>: quarterly to target LTV {(inp.ltv * 100).toFixed(0)}%. <em>Auto‑draw</em> {autoDrawToTarget ? "ON" : "OFF"} — ON: uses capacity as it accrues; OFF: waits until purchase.</li>
                  <li><strong>Referrals</strong>: growth in <em>positive</em> P&L of referred users boosts your pension as a 1% fee (referral params: count and avg monthly contribution).</li>
                  ...
                  <li><strong>Result</strong>: shows Net Worth (with/without referrals) and P&L vs. contributions; 1% platform + 1% referrer fee on <em>positive</em> P&L.</li>
                </ul>
              </div>
              {(spreadWarn || ltvWarn) && (
                <div className="text-xs text-amber-300">
                  {spreadWarn && <p>Warning: Spread ≤ 0 (yield ≤ interest). Even under ideal conditions, yield does not cover loan cost.</p>}
          ...
          <section className="grid xl:grid-cols-3 lg:grid-cols-2 gap-10">
            ...
            <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
              <CardHeader className="px-6 pt-6 pb-3">
                ...
                <CardDescription className="text-xs text-gray-400">
                  Enter % fields as <em>percent values</em>; the model converts to fractions.
                </CardDescription>
              </CardHeader>
              <CardContent className="px-6 pb-8 space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  {([
                    ["monthlyContribution", "Monthly Contribution (EUR)", 1, 1, "Monthly EUR contribution, used to buy BTC at each month's price"],
                    ["initialPrice", "Initial BTC Price (EUR)", 1, 1, "Starting BTC price in EUR"],
                    ["cagr", "BTC CAGR (%)", 0.1, 100, "Annual BTC growth rate in the ideal scenario (no volatility)"],
                    ["years", "Horizon (yrs)", 1, 1, "Savings horizon in years"],
                    ["ltv", "Max/Target LTV (%)", 0.1, 100, "Target/maximum LTV used for loan capacity calculations"],
                    ["loanRate", "Loan Interest (APR, %)", 0.1, 100, "Annual loan interest rate"],
                    ["yieldRate", "Yield Rate (APY, %)", 0.1, 100, "Annual yield on deployed capital (on loan)"],
                  ] as [keyof SimulationInput, string, number, number, string][]).map(
                    ([k, label, step, mult, tip]) => (
                      <div key={k} className="space-y-1">
                        <LabelWithInfo text={label} tip={tip} />
                        <Input
                          type="number"
                          min={0}
                          step={step}
                ...
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div className="space-y-1">
                    <LabelWithInfo text="Auto‑draw to Target LTV" tip="Automatically draw loan balance to target LTV at quarter end. When OFF — loan capacity accumulates until a purchase." />
                    <Button
                      type="button"
                  ...
                  <div className="space-y-1">
                    <LabelWithInfo text="Referrals — Count" tip="Number of direct referred users, saving with the same parameters (except their own monthly contribution)." />
                    <Input
                      type="number"
                      min={0}
                      value={refCount}
                ...
                <div className="grid grid-cols-3 gap-4 pt-4">
                  <div className="space-y-1">
                    <LabelWithInfo text="Car Cost (EUR)" tip="Budget for a car financed by a loan collateralized by your BTC." />
                    <Input
                      type="number"
                      min={0}
                      value={carCost}
                ...
                <div className="grid grid-cols-2 gap-4 pt-4">
                  <div className="space-y-1">
                    <LabelWithInfo text="Referrals — Avg Monthly (EUR)" tip="Average monthly contribution of a referred user. Used to model their P&L and resulting 1% fee." />
                    <Input
                      type="number"
                      min={0}
                      value={refAvgMonthly}
                ...
                <div className="grid grid-cols-3 gap-2 pt-3 text-xs text-gray-300">
                  <span>
                ...
                <Button
                  className="w-full bg-orange-600 hover:bg-orange-500 text-white rounded-lg shadow-md mt-2"
                  onClick={() => setInp({ ...inp })}
                >
                  Recalculate
                </Button>
                ...
                <div className="grid grid-cols-2 gap-2 pt-4 text-xs text-gray-300">
                  ...
                  <span>
                    Total Referral Income: <strong>€{fmt(totalRefIncome)}</strong>
                  </span>
                  <span>
                    Net Worth + Referrals: <strong>€{fmt((simWithRef[simWithRef.length - 1]?.netWorthWithRef) || last.netWorth)}</strong>
                  </span>
                  <span>
            ...
            <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
              <CardHeader className="px-6 pt-6 pb-3">
                ...
                <CardDescription className="text-xs text-gray-400">
                  BTC value, loan & net worth over {inp.years} yrs + purchase thresholds + referral uplift
                </CardDescription>
              </CardHeader>
            ...
            <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
              <CardHeader className="px-6 pt-6 pb-3">
                ...
                <CardDescription className="text-xs text-gray-400">
                  1% fee after {inp.years} years vs user count (from positive P&L)
                </CardDescription>
              </CardHeader>
              <CardContent className="px-6 pb-8 space-y-4 h-80">
                <div>
                  <LabelWithInfo
                    text="Max Users"
                    tip="Total number of active users at horizon — platform earnings chart (1% of user P&L)."
                  />
                  <Input
                    type="number"
                    min={1}
            ...
            <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
              <CardHeader className="px-6 pt-6 pb-3">
                ...
                <CardDescription className="text-xs text-gray-400">
                  1% share after {inp.years} years vs number of referrals (from positive P&L)
                </CardDescription>
              </CardHeader>
              <CardContent className="px-6 pb-8 space-y-4 h-80">
                <div>
                  <LabelWithInfo
                    text="Max Referrals"
                    tip="Number of direct referrals — referrer earnings chart (1% of user P&L)."
                  />
                  <Input
                    type="number"
                    min={1}
