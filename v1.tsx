...
const simulate = (params: SimulationInput): SimulationPoint[] => {
  ...
  for (let m = 0; m <= months; m++) {
    const price =
      initialPrice * Math.pow(1 + cagr, m / 12); // exponential growth
      ...
      <Card className="w-full">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">Initial BTC Price (€)</label>
              <Input
                type="number"
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => handleChange("cagr", String(Number(e.target.value) / 100))}
                min={0}
                step={0.1}
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                type="number"
                value={input.ltv * 100}
                onChange={(e) => handleChange("ltv", String(Number(e.target.value) / 100))}
                min={0}
                max={90}
              />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Loan Interest (% p.a.)</label>
              <Input
                type="number"
                value={input.loanRate * 100}
                onChange={(e) => handleChange("loanRate", String(Number(e.target.value) / 100))}
                min={0}
                step={0.1}
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => handleChange("yieldRate", String(Number(e.target.value) / 100))}
                min={0}
                step={0.1}
              />
            </div>
          </div>
          <Button onClick={() => setInput({ ...input })} className="w-full mt-4">
            Recalculate
          </Button>
          {/* Summary */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <p>
              <strong>Total BTC accumulated:</strong> {finalPoint.btcHolding.toFixed(4)} BTC
            </p>
            <p>
              <strong>BTC value at year {input.years}:</strong> €{finalPoint.btcValue.toFixed(0)}
            </p>
            <p>
              <strong>Loan balance (30% LTV):</strong> €{finalPoint.loan.toFixed(0)}
            </p>
            <p>
              <strong>Cumulative net cash‑flow:</strong> €{finalPoint.cumulativeCashFlow.toFixed(0)}
            </p>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="h-96">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
              ...
              <Line type="monotone" dataKey="loan" strokeWidth={2} name="Loan Balance (€)" strokeDasharray="5 5" />
              <Line type="monotone" dataKey="cumulativeCashFlow" strokeWidth={2} name="Cum. Cash‑flow (€)" />
            </LineChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
