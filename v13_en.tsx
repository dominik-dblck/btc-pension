...
const BTC_PensionCalculator: React.FC = () => {
  ...
  const carM = useMemo(() => firstAffordableMonthWithRef(sim, carCost, inp.ltv, refFeeByMonth, inp.enableIndexing), [sim, carCost, inp.ltv, refFeeByMonth, inp.enableIndexing]);
  const houseM = useMemo(() => firstAffordableMonthWithRef(sim, houseCost, inp.ltv, refFeeByMonth, inp.enableIndexing), [sim, houseCost, inp.ltv, refFeeByMonth, inp.enableIndexing]);
  const tuitionM = useMemo(() => firstAffordableMonthWithRef(sim, tuitionCost, inp.ltv, refFeeByMonth, inp.enableIndexing), [sim, tuitionCost, inp.ltv, refFeeByMonth, inp.enableIndexing]);
        ...
        >
          ...
          <header className="text-center space-y-4">
            ...
            <p className="text-sm md:text-base text-gray-300 max-w-3xl mx-auto">
              Deterministic DCA → LTV‑loan → yield model under ideal conditions. Thresholds for major purchases (car / house / tuition) are calculated from available loan capacity.
            </p>
            ...
            <CardContent className="px-6 pb-8 space-y-6 text-sm">
              <div>
                ...
                <ul className="list-disc pl-6 space-y-1 text-gray-300">
                  ...
                  <li><strong>Collateral</strong>: loan capacity = <em>LTV × BTC value − loan balance</em>. Purchases (car/house/tuition) possible when capacity ≥ cost.</li>
                  ...
                  <li><strong>Referrals</strong>: 1% fee from <em>positive</em> P&L growth of referred users boosts your pension (referral params: count and avg monthly). <strong>Now included in purchase thresholds.</strong></li>
          ...
          <section className="grid lg:grid-cols-2 gap-10 items-start">
            ...
            <Card ref={paramsCardRef} className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
              ...
              <CardContent className="px-6 pb-8 space-y-6">
                ...
                {/* Using capital */}
                <div>
                  <h4 className="text-sm font-semibold text-white mb-2">Using capital (house / tuition / car)</h4>
                  <div className="grid md:grid-cols-3 gap-4">
                    <div className="space-y-1">
                      <LabelWithInfo text="Car Cost (EUR)" tip="Budget for a car financed by a loan against your BTC." />
                      <Input type="number" min={0} value={carCost} onChange={(e) => setCarCost(Number(e.target.value))} className="rounded-lg bg-slate-900/70 border-slate-700 text-white text-xs p-[2px]" />
                    </div>
                    <div className="space-y-1">
                      <LabelWithInfo text="House Cost (EUR)" tip="Budget for a house/apartment; model checks when loan capacity reaches this cost at the given LTV." />
                      <Input type="number" min={0} value={houseCost} onChange={(e) => setHouseCost(Number(e.target.value))} className="rounded-lg bg-slate-900/70 border-slate-700 text-white text-xs p-[2px]" />
                    </div>
                    <div className="space-y-1">
                      <LabelWithInfo text="Tuition Cost (EUR)" tip="Total cost of children's tuition; moment when it can be financed by a BTC-backed loan." />
                      <Input type="number" min={0} value={tuitionCost} onChange={(e) => setTuitionCost(Number(e.target.value))} className="rounded-lg bg-slate-900/70 border-slate-700 text-white text-xs p-[2px]" />
                    </div>
                  </div>
                </div>
            ...
            >
              ...
              <CardContent className="px-6 pb-8 space-y-3">
                <p className="text-xs text-gray-400">BTC value, loan & net worth over {inp.years} yrs + purchase thresholds + referral uplift + inflation adjustment</p>
                <div className="w-full" style={{ height: portfolioHeight }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={simWithRef}>
                      ...
                      <Line yAxisId="left" type="monotone" dataKey="btcValue" strokeWidth={2} name="BTC Value (€)" />
                      <Line yAxisId="left" type="monotone" dataKey="loanOutstanding" strokeWidth={2} strokeDasharray="5 5" name="Loan (€)" />
                      <Line yAxisId="left" type="monotone" dataKey="netWorth" strokeWidth={2} name="Net Worth (no refs) (€)" />
                      <Line yAxisId="left" type="monotone" dataKey="netWorthWithRef" strokeWidth={2} strokeDasharray="2 2" name="Net Worth + Referrals (€)" />
                      <Line yAxisId="left" type="monotone" dataKey="realNetWorth" strokeWidth={1} strokeOpacity={0.7} name="Real Net Worth (no refs) (€)" />
                      <Line yAxisId="left" type="monotone" dataKey="realNetWorthWithRef" strokeWidth={1} strokeOpacity={0.7} strokeDasharray="1 1" name="Real Net Worth + Referrals (€)" />
                      <Line yAxisId="right" type="monotone" dataKey="contribution" strokeWidth={1.5} strokeDasharray="4 2" name="Monthly Contribution (€)" /> {/* NEW */}
                      {carM !== null && <ReferenceLine yAxisId="left" x={carM} strokeDasharray="3 3" label="Car 🚗" />}
                      {houseM !== null && <ReferenceLine yAxisId="left" x={houseM} strokeDasharray="3 3" label="House 🏠" />}
                      {tuitionM !== null && <ReferenceLine yAxisId="left" x={tuitionM} strokeDasharray="3 3" label="Tuition 🎓" />}
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
                  ...
                  <div className="w-full h-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={simWithRef}>
                        ...
                        <Line yAxisId="left" type="monotone" dataKey="realNetWorthWithRef" strokeWidth={1} strokeOpacity={0.7} strokeDasharray="1 1" name="Real Net Worth + Referrals (€)" />
                        <Line yAxisId="right" type="monotone" dataKey="contribution" strokeWidth={1.5} strokeDasharray="4 2" name="Monthly Contribution (€)" /> {/* NEW */}
                        {carM !== null && <ReferenceLine yAxisId="left" x={carM} strokeDasharray="3 3" label={<span><div style={{fontSize: "1.2em"}}>🚗</div>Car</span>} />}
                        {houseM !== null && <ReferenceLine yAxisId="left" x={houseM} strokeDasharray="3 3" label={<span><div style={{fontSize: "1.2em"}}>🏠</div>House</span>} />}
                        {tuitionM !== null && <ReferenceLine yAxisId="left" x={tuitionM} strokeDasharray="3 3" label={<span><div style={{fontSize: "1.2em"}}>🎓</div>Tuition</span>} />}
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </div>
          ...
          <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
            ...
            <CardContent className="px-6 pb-8 space-y-3">
              <p className="text-xs text-gray-400">1% fee after {inp.years} years — control <em>Users — Count</em> and <em>Users — Avg Monthly</em> independently.</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
...
function runSelfCheck() {
  ...
  console.assert(firstAffordableMonthWithRef(s, 1e6, base.ltv, feesMap, base.enableIndexing) === 0, "ref fees should improve affordability");
}
try {
