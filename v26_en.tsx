...
const yrs = (m: number | null) => (m === null ? "—" : m ? `${(m / 12).toFixed(1)} yrs` : "—");
...
try {
  if (typeof window !== "undefined") {
    ...
    const months = p2.years * 12;
    let expectedReal = 0;
    for (let m = 1; m <= months; m++) {
      const infl = Math.pow(1 + p2.cpiRate, m / 12);
      const gross = p2.monthlyContribution; // enableIndexing=false
      expectedReal += gross / infl;
    }
