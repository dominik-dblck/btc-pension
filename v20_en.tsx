...
const BTC_PensionCalculator: React.FC = () => {
  ...
  // per-user fee after full horizon — YIELD-ONLY
  ...
  // NEW: annual platform revenue path (cohort model) — YIELD-ONLY fee
  const platformAnnualSeries = useMemo(() => {
    ...
    const out = Array.from({ length: years }, (_, i) => {
      ...
      for (let age = 1; age <= Y; age++) {
        ...
        const feeAge = feePUByAge.get(age) ?? 0;
        total += cohortSize * feeAge;
      }
      const users = usersSeries[i] ?? 0;
      const avgPerUser = users > 0 ? total / users : 0;
      return { year: Y, users, newUsers: newUsersSeries[i] ?? 0, total, avgPerUser };
    });
    return out;
  }, [inp, platformAvgMonthly, platformEnableIndexing, platformFeePct, autoDrawToTarget, usersStart, usersEnd, usersGrowthMode]);
            ...
            <CardContent className="px-6 pb-8 space-y-6 text-sm">
              <div>
                ...
                <ul className="list-disc pl-6 space-y-1 text-gray-300">
                  ...
                  <li><strong>Platform fee (yield-only)</strong>: platform charges {platformFeePct}% exclusively from <em>gross yield</em> on deployed capital (loan). No fees from BTC price / P&amp;L.</li>
                </ul>
              </div>
              {(spreadWarn || ltvWarn) && (
          ...
          <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
            ...
            <CardContent className="px-6 pb-8 space-y-3">
              ...
              <p className="text-xs text-gray-400 pt-1">
                Chart: <em>Total (€/year)</em>, <em>Avg per user (€/year)</em> and <em>Users</em> (right axis).
                Fees calculated exclusively from yield (earn) — cohort convolution (users join over time).
              </p>
              <div className="h-[380px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={platformAnnualSeries}>
                    ...
                    <Line yAxisId="left" type="monotone" dataKey="avgPerUser" strokeWidth={2} strokeDasharray="4 2" stroke="#22c55e" name="Avg per user (€/year)" />
