  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-8 space-y-8">
      ...
      <Card className="shadow-xl border border-gray-200">
        <CardHeader>
          <CardTitle className="text-2xl">BTC Pension & Passive‑Income Simulator</CardTitle>
          <CardDescription>
            Build a long‑term Bitcoin pension while generating fiat cash‑flow from a low‑risk collateral
            strategy. Quarterly sampling & max‑120‑point charts keep the UI smooth.
          </CardDescription>
        </CardHeader>
        <CardContent className="text-sm leading-relaxed space-y-2">
          <p className="font-medium">Key assumptions:</p>
          <ul className="list-disc pl-6 space-y-1">
            <li>Monthly fiat contribution buys BTC (DCA). As the BTC price rises (<strong>{(inp.cagr * 100).toFixed(1)}% CAGR</strong>), each € buys fewer sats.</li>
            <li><strong>{(inp.ltv * 100).toFixed(0)}% LTV</strong> loan at {(inp.loanRate * 100).toFixed(1)}% APR; capital redeployed at {(inp.yieldRate * 100).toFixed(1)}% APY.</li>
            <li>Platform & referrer each collect a <strong>1 % performance fee</strong> on the user’s net profit.</li>
            <li>All projections span <strong>{inp.years} years</strong>; values are before tax.</li>
          </ul>
        </CardContent>
      </Card>
      ...
      <div className="grid gap-8 xl:grid-cols-3 lg:grid-cols-2">
        ...
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <CardDescription>Hover fields for hints; press recalculate to apply.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              {([
                ["monthlyContribution", "Monthly € contributed", 1, 1, "Fiat amount auto‑converted to BTC each month."],
                ["initialPrice", "Initial BTC Price €", 1, 1, "Spot price today (starting point)."],
                ["cagr", "BTC CAGR %", 0.1, 100, "Expected average annual BTC appreciation."],
                ["years", "Horizon years", 1, 1, "Total investment period."],
                ["ltv", "Loan‑to‑Value %", 0.1, 100, "% of BTC value taken as collateralised loan."],
                ["loanRate", "Loan Interest % p.a.", 0.1, 100, "Annual interest on the fiat loan."],
                ["yieldRate", "Yield Rate % p.a.", 0.1, 100, "Annual yield on deployed fiat capital."],
              ] as [keyof SimulationInput, string, number, number, string][]).map(([k, label, step, mult, tip]) => (
                <div key={k} className="space-y-1">
                  <label className="text-sm font-medium" title={tip}>{label}</label>
                  <Input
                    type="number"
                    min={0}
                    step={step}
                    value={(inp[k] as number) * mult}
                    onChange={(e) => upd(k, String(Number(e.target.value) / mult))}
                    title={tip}
                  />
                </div>
              ))}
            </div>
            <Button className="w-full" variant="secondary" onClick={() => setInp({ ...inp })}>Recalculate</Button>
            <hr />
            <div className="grid grid-cols-2 text-sm gap-2">
              ...
              <p><strong>User profit:</strong> €{fmt(last.cumulativeCashFlow)}</p>
              <p><strong>Fee / user:</strong> €{feePU.toFixed(2)}</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <CardDescription>BTC value, loan balance & cumulative cash‑flow</CardDescription>
          </CardHeader>
          <CardContent className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={sim}>
                ...
                <Line type="monotone" dataKey="loan" name="Loan (€)" strokeWidth={2} strokeDasharray="5 5" />
        ...
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <CardDescription>Total 1 % fee after {inp.years} years vs users</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 h-80">
            <div>
              <label className="text-sm font-medium" title="Size of the overall user base.">Max Users</label>
        ...
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <CardDescription>Passive 1 % share after {inp.years} years vs referrals</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 h-80">
            <div>
              <label className="text-sm font-medium" title="Number of direct invitees.">Max Referrals</label>
