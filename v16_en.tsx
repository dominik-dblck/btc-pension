    ...
    <TooltipProvider>
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-black text-gray-100">
        ...
        >
          ...
          <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
            <CardHeader className="px-6 pt-6 pb-3">
              <CardTitle className="text-lg text-white">Platform Earnings — Over Time (Annual)</CardTitle>
            </CardHeader>
            <CardContent className="px-6 pb-8 space-y-3">
              <p className="text-xs text-gray-400">
                Roczne przychody z fee od dodatnich przyrostów P&amp;L (high‑water mark). Linie: per user oraz łącznie dla <strong>{fmt(maxUsers)}</strong> users. Fee: <strong>{platformFeePct}%</strong>.
              </p>
              <div className="h-[380px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={platformAnnualSeries}>
                    ...
                    <RechartsTooltip
                      ...
                      labelFormatter={(y) => `Year: ${y} • Users: ${fmt(maxUsers)}`}
                      contentStyle={{ background: "#0b1220", color: "#e5e7eb", borderRadius: 8, border: "1px solid #334155", boxShadow: "0 10px 25px rgba(0,0,0,0.35)" }}
                    />
                    <Legend wrapperStyle={{ fontSize: 10 }} />
                    <Line type="monotone" dataKey="perUser" strokeWidth={2} stroke="#22c55e" name="Per user (€/year)" />
                    <Line type="monotone" dataKey="total" strokeWidth={2} strokeDasharray="4 2" stroke="#06b6d4" name={`Total (€/year) @ ${fmt(maxUsers)} users`} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
