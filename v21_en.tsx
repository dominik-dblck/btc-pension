...
function exchangeFeePerUser(
  ...
  for (let m = 1; m <= months; m++) {
    ...
    // Exchange fee = % of BTC value (monthly)
    const btcValue = btcHolding * price;
    exchangeFeePU += btcValue * (exchangeFeePct / 100);
...
function exchangeFeePerUserByAgeYear(
  ...
  for (let m = 1; m <= months; m++) {
    ...
    const btcValue = btcHolding * price;
    const ageYear = Math.floor((m - 1) / 12) + 1;
    const add = btcValue * (exchangeFeePct / 100);
    out.set(ageYear, (out.get(ageYear) ?? 0) + add);
...
const BTC_PensionCalculator: React.FC = () => {
  ...
  // Exchange fee = % of BTC value — default 0.1%
  ...
  const platformFeePU = useMemo(() => {
    return feePerUserFromYield(
      { ...inp, monthlyContribution: platformAvgMonthly, enableIndexing: platformEnableIndexing },
      platformFeePct,
      autoDrawToTarget,
      platformEnableIndexing
    );
  }, [inp, platformAvgMonthly, platformEnableIndexing, platformFeePct, autoDrawToTarget]);
            ...
            <CardContent className="px-6 pb-8 space-y-6 text-sm">
              <div>
                ...
                <ul className="list-disc pl-6 space-y-1 text-gray-300">
                  ...
                  <li><strong>Platform fees</strong>: yield fee {platformFeePct}% from <em>gross yield</em> on deployed capital (loan) + exchange fee {exchangeFeePct}% from <em>BTC value</em> (monthly) + {exchangeFeePct}% fee when exchanging EUR to BTC. No fees from BTC price growth / P&amp;L.</li>
                </ul>
              </div>
              {(spreadWarn || ltvWarn) && (
            ...
            <CardContent className="px-6 pb-8 space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-1">
                  <LabelWithInfo
                    text="Exchange Fee (%)"
                    tip="Percentage charged by the platform from total BTC value (monthly). This represents exchange/custody fees."
                  />
              ...
              <p className="text-xs text-gray-400 pt-1">
                ...
                Fees calculated from yield (earn) + BTC value — cohort convolution (users join over time).
              </p>
...
try {
  if (typeof window !== "undefined") {
    ...
    const fee0 = feePerUserFromYield(p, 10, true);
