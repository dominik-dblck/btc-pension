...
const simulate = (
  ...
  for (let m = 0; m <= months; m++) {
    ...
    if (m > 0) {
      const gross = p.enableIndexing ? p.monthlyContribution * inflationIndex : p.monthlyContribution;
...
function feePerUserFromYield(
  ...
  for (let m = 1; m <= months; m++) {
    ...
    const gross = indexing ? p.monthlyContribution * inflationIndex : p.monthlyContribution;
    const fee = gross * ((p.exchangeFeePct ?? 0) / 100);
    ...
    if (isQuarterEnd) {
      ...
      if (autoDrawToTarget) {
        ...
        if (delta > 0) { loanOutstanding += delta; cashBalance += delta; }
        else if (delta < 0) {
          const repay = Math.min(-delta, cashBalance);
          loanOutstanding -= repay; cashBalance -= repay;
        }
      } else {
        if (loanOutstanding > targetLoan) {
          const repay = Math.min(loanOutstanding - targetLoan, cashBalance);
...
function exchangeFeePerUser(
  ...
  for (let m = 1; m <= months; m++) {
    ...
    const indexing = enableIndexingOverride ?? p.enableIndexing;
    const gross = indexing ? p.monthlyContribution * inflationIndex : p.monthlyContribution;
    const contribution = gross - (gross * ((p.exchangeFeePct ?? 0) / 100));
...
const BTC_PensionCalculator: React.FC = () => {
  ...
  const refSim = useMemo(
    () => simulate({ ...inp, monthlyContribution: refAvgMonthly, exchangeFeePct }, autoDrawToTarget),
  ...
  const platformFeePU = useMemo(() => {
    return feePerUserFromYield(
      { ...inp, monthlyContribution: platformAvgMonthly, enableIndexing: platformEnableIndexing, exchangeFeePct },
      platformFeePct,
      autoDrawToTarget,
      platformEnableIndexing
    );
  }, [inp, platformAvgMonthly, platformEnableIndexing, platformFeePct, exchangeFeePct, autoDrawToTarget]);
  ...
  }, [inp, platformAvgMonthly, platformEnableIndexing, exchangeFeePct, autoDrawToTarget]);

  // referrer earnings chart series (separate avg monthly)
  const refChartSim = useMemo(
    () => simulate({ ...inp, monthlyContribution: refChartAvgMonthly, exchangeFeePct }, autoDrawToTarget),
    [inp, refChartAvgMonthly, exchangeFeePct, autoDrawToTarget]
  );
  const refChartFeePU = refChartSim.reduce((sum, pt) => sum + pt.yieldEarned, 0) * 0.05;
  ...
  const platformAnnualSeries = useMemo(() => {
    ...
    const newUsersSeries = usersSeries.map((u, i) => Math.max(0, u - (i > 0 ? usersSeries[i - 1] : 0)));
    ...
    return out;
  }, [inp, platformAvgMonthly, platformEnableIndexing, platformFeePct, exchangeFeePct, autoDrawToTarget, usersStart, usersEnd, usersGrowthMode]);
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-black text-gray-100">
        ...
        >
            ...
            <CardContent className="px-6 pb-8 space-y-6 text-sm">
              <div>
                ...
                <ul className="list-disc pl-6 space-y-1 text-gray-300">
                  ...
                  <li><strong>Referrals</strong>: 5% fee from <em>yield earned</em> by referred users (used only for "Referrer Earnings").</li>
                  ...
                  <li><strong>Platform fees</strong>: yield fee {platformFeePct}% from <em>gross yield</em> on deployed capital (loan) + exchange fee {exchangeFeePct}% charged <em>once</em> on every EUR → BTC purchase. No fees from BTC price growth / P&amp;L.</li>
                </ul>
              </div>
              {(spreadWarn || ltvWarn) && (
            ...
            <Card ref={paramsCardRef} className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
              ...
              <CardContent className="px-6 pb-8 space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <LabelWithInfo text="Monthly Contribution (EUR, gross)" tip="EUR you send every month *before* exchange fee; fee is deducted and only the remainder buys BTC." />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <LabelWithInfo text="Loan Amount (EUR)" tip="Amount of BTC-collateralized loan, full coverage according to selected LTV." />
                    ...
                    <div className="space-y-1">
                      <LabelWithInfo text="Loan LTV (%)" tip="LTV used to assess capacity: capacity = LTV × BTC value − existing debt." />
                ...
                <div className="w-full" style={{ height: portfolioHeight }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={simWithRef}>
                      ...
                      <Line yAxisId="right" type="monotone" dataKey="contribution" stroke="#a78bfa" strokeWidth={1.8} strokeDasharray="4 2" name="Monthly Contribution (net €)" />
          ...
          <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
            <CardHeader className="px-6 pt-6 pb-3">
              <CardTitle className="text-lg text-white">Platform Revenue — Annual (Cohorts & User Growth)</CardTitle>
            </CardHeader>
            <CardContent className="px-6 pb-8 space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-1">
                                        <LabelWithInfo text="Users — Start (Year 1)" tip="Number of users in the first year (e.g. 50,000)." />
                ...
                <div className="space-y-1">
                                        <LabelWithInfo text="Users — End (Year N)" tip="Target number of users in the last year (e.g. 1,000,000)." />
                ...
                <div className="space-y-1">
                                        <LabelWithInfo text="Growth Mode" tip="Linear vs CAGR (exponential) user growth." />
                  <Button type="button"
                ...
                <div className="space-y-1">
                  ...
                  />
                  <Input type="number" min={0} step={0.1} value={platformFeePct} onChange={(e) => setPlatformFeePct(Number(e.target.value))}
                         className="rounded-lg bg-slate-900/70 border-slate-700 text-white text-xs p-[2px]" />
                </div>
                <div className="space-y-1">
                  <LabelWithInfo
                  ...
                  />
                  <Input type="number" min={0} step={0.01} value={exchangeFeePct} onChange={(e) => setExchangeFeePct(Number(e.target.value))}
                         className="rounded-lg bg-slate-900/70 border-slate-700 text-white text-xs p-[2px]" />
                </div>
                <div className="space-y-1">
                                        <LabelWithInfo text="Users — Avg Monthly (EUR)" tip="Average monthly deposit of platform user (affects P&amp;L, and therefore fees)." />
                  <Input type="number" min={0} value={platformAvgMonthly} onChange={(e) => setPlatformAvgMonthly(Number(e.target.value))}
                         className="rounded-lg bg-slate-900/70 border-slate-700 text-white text-xs p-[2px]" />
                </div>
                <div className="space-y-1">
                                        <LabelWithInfo text="Platform Indexing (CPI)" tip="When ON — platform user contributions grow with inflation (affects P&amp;L path and fees over years)." />
                  <Button type="button"
              ...
              <p className="text-xs text-gray-400 pt-1">
                Chart: <em>Total (€/year)</em>, <em>Yield Fee (€/year)</em>, <em>Exchange Fee (€/year)</em>, <em>Avg per user (€/year)</em> and <em>Users</em> (right axis).
                Fees calculated from yield (earn) + one‑off exchange fees (EUR→BTC purchases) — cohort convolution (users join over time).
              </p>
                  ...
                  <LineChart data={platformAnnualSeries}>
                    ...
                    <RechartsTooltip
                      formatter={(val: number, name: string) => {
                        if (name === "Users" || name === "New users") return [fmt(val), name];
                        return [`€${fmt(val)}`, name];
                      }}
                    ...
                    />
                    <Legend wrapperStyle={{ fontSize: 10 }} />
                    <Line yAxisId="left" type="monotone" dataKey="total" strokeWidth={2} stroke="#06b6d4" name="Total (€/year)" />
                    <Line yAxisId="left" type="monotone" dataKey="totalYieldFee" strokeWidth={2} stroke="#22c55e" name="Yield Fee (€/year)" />
                    <Line yAxisId="left" type="monotone" dataKey="totalExchangeFee" strokeWidth={2} stroke="#f59e0b" name="Exchange Fee (€/year)" />
                    <Line yAxisId="left" type="monotone" dataKey="avgPerUser" strokeWidth={2} strokeDasharray="4 2" stroke="#a78bfa" name="Avg per user (€/year)" />
                    <Line yAxisId="right" type="monotone" dataKey="users" strokeWidth={1.8} stroke="#f472b6" name="Users" />
          ...
          <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
            ...
            <CardContent className="px-6 pb-8 space-y-3">
              <p className="text-xs text-gray-400">5% share from yield earned over {inp.years} years — control <em>Referrals — Count</em> and <em>Referrals — Avg Monthly</em> independently for this chart.</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
...
try {
  if (typeof window !== "undefined") {
    ...
    console.assert(Math.abs(fee0) < 1e-8, "Platform fee should be 0 when yieldRate=0 (even if BTC price grows).");
    
    // Exchange fee test: with 1% fee, totalContribReal should include gross amounts
    const p2: SimulationInput = {
      monthlyContribution: 100,
      initialPrice: 50_000,
      cagr: 0.1,
      years: 1,
    ...
    const s2 = simulate({ ...p2, exchangeFeePct: 1 }, false);
    const last2 = s2[s2.length - 1];
    // With 1% exchange fee, each 100 EUR contribution becomes 99 EUR net
    // totalContribReal should be 12 * 100 / inflationIndex (not 12 * 99)
    const expectedGross = 12 * 100;
    const expectedReal = expectedGross / Math.pow(1 + p2.cpiRate, 1);
    const diff = Math.abs(last2.totalContribReal - expectedReal);
    console.assert(diff / Math.max(1, expectedReal) < 1e-9, "totalContribReal should include gross amounts when exchange fee > 0");
  }
} catch {}
