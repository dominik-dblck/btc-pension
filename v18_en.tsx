...
const BTC_PensionCalculator: React.FC = () => {
  ...
  // base for per-user fees: representative platform user (with their own indexing)
  const platformSim = useMemo(
    () => simulate(
      { ...inp, monthlyContribution: platformAvgMonthly, enableIndexing: platformEnableIndexing },
      autoDrawToTarget
    ),
    [inp, platformAvgMonthly, platformEnableIndexing, autoDrawToTarget]
  );
  // per-user fee after full cycle (horizon), dependent on regulated Fee (%)
  ...
  // NEW: annual platform revenue path (cohort model)
  const platformAnnualSeries = useMemo(() => {
          // 1) fee per-user by "age year": high‑water mark from representative user
    let prevPeak = 0;
            const feePUByAge = new Map<number, number>(); // ageYear -> fee per user in that age year
    for (const pt of platformSim) {
      const pnlPlus = Math.max(0, pt.pnlNet);
      const inc = Math.max(0, pnlPlus - prevPeak);
      if (inc > 0) {
        const ageYear = Math.floor(pt.month / 12) + 1; // 1..inp.years
        const feeStep = inc * (platformFeePct / 100);
        feePUByAge.set(ageYear, (feePUByAge.get(ageYear) ?? 0) + feeStep);
        prevPeak = pnlPlus;
      }
    }
            ...
            <CardContent className="px-6 pb-8 space-y-6 text-sm">
              <div>
                ...
                <ul className="list-disc pl-6 space-y-1 text-gray-300">
                  ...
                  <li><strong>Referrals</strong>: 1% fee from <em>positive</em> P&amp;L increments of referred users increases your balance (parameters: count and average deposit).</li>
          ...
          <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
            ...
            <CardContent className="px-6 pb-8 space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-1">
                                        <LabelWithInfo text="Fee (%)" tip="Percentage of participant's positive P&amp;L (high‑water mark) collected by the platform." />
              ...
              <p className="text-xs text-gray-400 pt-1">Chart: <em>Total (€/year)</em>, <em>Avg per user (€/year)</em> and <em>Users</em> (right axis). Cohort view (users join over time).</p>
              <div className="h-[380px] w-full">
                <ResponsiveContainer width="100%" height="100%">
