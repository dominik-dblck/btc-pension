 ...
 *  - optional auto-draws to target LTV at END of each quarter (toggle),
  ...
  for (let m = 0; m <= months; m++) {
    ...
    if (isQuarterEnd) {
      ...
      if (autoDrawToTarget) {
        ...
        // conservative: only repay if above target (e.g., po spadku ceny)
      ...
      >
        ...
        <header className="text-center space-y-4">
          ...
          <p className="text-sm md:text-base text-gray-300 max-w-3xl mx-auto">
            Deterministyczny model DCA → LTV‑loan → yield pod idealne warunki. Dodano progi (thresholds) na duże wydatki finansowane kredytem pod zastaw BTC.
          </p>
        </header>
          ...
          <CardContent className="px-6 pb-8 space-y-6 text-sm">
            <div>
              ...
              <ul className="list-disc pl-6 space-y-1 text-gray-300">
                <li>DCA miesięczne w EUR; deterministyczny CAGR BTC {(inp.cagr * 100).toFixed(1)}%.</li>
                <li>Docelowy/maksymalny LTV {(inp.ltv * 100).toFixed(0)}%, pożyczka APR {(inp.loanRate * 100).toFixed(1)}%, yield APY {(inp.yieldRate * 100).toFixed(1)}%.</li>
                <li>Rebalans kwartalny. <strong>Auto‑draw do LTV</strong>: {autoDrawToTarget ? "ON (wykorzystuj pojemność długu na bieżąco)" : "OFF (zaciągamy dług tylko przy zakupach)"}.</li>
                <li>Fee: 1% platforma + 1% referrer od pozytywnego P&amp;L netto użytkownika.</li>
              </ul>
            </div>
            {(spreadWarn || ltvWarn) && (
        ...
        <section className="grid xl:grid-cols-3 lg:grid-cols-2 gap-10">
          ...
          <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
            <CardHeader className="px-6 pt-6 pb-3">
              ...
              <CardDescription className="text-xs text-gray-400">
                Pola % są wprowadzane jako wartości <em>w procentach</em>; model pracuje na ułamkach.
              </CardDescription>
            </CardHeader>
            <CardContent className="px-6 pb-8 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                {([
                  ["monthlyContribution", "Monthly Contribution (EUR)", 1, 1, "Miesięczna wpłata w EUR"],
                  ["initialPrice", "Initial BTC Price (EUR)", 1, 1, "Cena startowa BTC"],
                  ["cagr", "BTC CAGR (%)", 0.1, 100, "Roczna aprecjacja BTC"],
                  ["years", "Horizon (yrs)", 1, 1, "Długość symulacji"],
                  ["ltv", "Max/Target LTV (%)", 0.1, 100, "Maksymalny LTV dla kredytu"],
                  ["loanRate", "Loan Interest (APR, %)", 0.1, 100, "Roczna stopa pożyczki"],
                  ["yieldRate", "Yield Rate (APY, %)", 0.1, 100, "Roczna stopa lokaty/yieldu"],
                ] as [keyof SimulationInput, string, number, number, string][]).map(
                  ([k, label, step, mult, tip]) => (
                    <div key={k} className="space-y-1">
                      <label className="text-xs font-semibold text-gray-300" title={tip}>
                        {label}
                      </label>
                      <Input
                        type="number"
              ...
              <div className="grid grid-cols-2 gap-4 pt-2">
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-gray-300" title="Auto‑draw do docelowego LTV na koniec kwartału">
                    Auto‑draw to Target LTV
                  </label>
                  <Button
                    type="button"
              ...
              <div className="grid grid-cols-3 gap-4 pt-4">
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-gray-300" title="Koszt samochodu (EUR)">Car Cost (EUR)</label>
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-gray-300" title="Koszt domu (EUR)">House Cost (EUR)</label>
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-gray-300" title="Łączny koszt studiów dzieci (EUR)">Tuition Cost (EUR)</label>
              ...
              <div className="grid grid-cols-2 gap-2 pt-4 text-xs text-gray-300">
                ...
                <span>Net Worth: <strong>€{fmt(last.netWorth)}</strong></span>
          ...
          <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
            <CardHeader className="px-6 pt-6 pb-3">
              ...
              <CardDescription className="text-xs text-gray-400">
                BTC value, loan & net worth over {inp.years} yrs + purchase thresholds
              </CardDescription>
            </CardHeader>
            <CardContent className="px-6 pb-8 h-80">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={sim}>
                  ...
                  <Line type="monotone" dataKey="netWorth" strokeWidth={2} name="Net Worth (€)" />
          ...
          <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
            <CardHeader className="px-6 pt-6 pb-3">
              ...
              <CardDescription className="text-xs text-gray-400">
                1% fee po {inp.years} latach vs liczba użytkowników (od pozytywnego P&L)
              </CardDescription>
            </CardHeader>
            <CardContent className="px-6 pb-8 space-y-4 h-80">
              <div>
                <label className="text-xs font-semibold text-gray-300" title="Łączna liczba aktywnych użytkowników po horyzoncie">
                  Max Users
                </label>
                <Input
                  type="number"
          ...
          <Card className="bg-slate-800/60 backdrop-blur rounded-2xl border border-slate-700 shadow-lg">
            <CardHeader className="px-6 pt-6 pb-3">
              ...
              <CardDescription className="text-xs text-gray-400">
                1% udział po {inp.years} latach vs liczba poleconych (od pozytywnego P&L)
              </CardDescription>
            </CardHeader>
            <CardContent className="px-6 pb-8 space-y-4 h-80">
              <div>
                <label className="text-xs font-semibold text-gray-300" title="Liczba bezpośrednich poleconych">
                  Max Referrals
                </label>
                <Input
                  type="number"
